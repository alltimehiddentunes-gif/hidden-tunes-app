...
async function getPlayableMp3(identifier: string) {
  try {
    ...
    console.log("Archive metadata error:", error);
    return null;
  }
}

export async function searchArchiveAudio(query: string): Promise<ArchiveTrack[]> {
  ...
  try {
    ...
    if (!text.trim().startsWith("{")) {
      console.log("Archive returned non-json response");
      return [];
    }
